...
- Speaker diarization is accelerator-only: Windows uses CUDA; macOS uses Apple Silicon CoreML for Speakrs or PyTorch Metal/MPS for pyannote. Linux Speakrs is CUDA-only on x86_64 after the accepted managed CUDA 12 preflight; there is no CPU fallback. Pyannote and llama.cpp summaries stay unavailable on Linux until later gates. Task 4 pins remain the Linux `speakrs-cli` and setup-time CUDA 12 ORT/model-pack closure in `src/ai-addon/speakrs-pack-spec.js`; do not weaken those pins.
