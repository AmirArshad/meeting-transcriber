...
**v2.9.0 (2026-08-31 refresh planned):** live decision record is [`docs/development/V2_9_DEPENDENCY_COMPATIBILITY.md`](V2_9_DEPENDENCY_COMPATIBILITY.md). Run a new availability snapshot across every npm direct dependency/override and every Python runtime floor/build pin; do not limit the release to already-open Dependabot PRs. Linux `filelock` remains aligned to `>=3.32.3` / `==3.32.3` until the refreshed platform evidence accepts a newer pin. macOS packaged `torch==2.13.0` + `setuptools==84.0.0` remains a resolver-only Torch pair (Torch is still pruned). Do **not** merge Dependabot PRs into `master` or `release/v2.9.0`; accept candidates only from that matrix after their platform gates. Stable Electron 44.0.0 is the dated latest-Electron candidate on `feature/v2.9-electron-44`; Electron 45 prereleases are excluded.

Last reviewed: 2026-08-31 (second availability sweep and latest-stable Electron 44.0.0 lane planned; no package version accepted by this documentation change). Prior: 2026-08-29 (Task 3 Windows/Linux remaining floats protobuf/Pygments/annotated-doc). Prior: 2026-08-29 (Task 3 Windows/Linux typer 0.27.2). Prior: 2026-08-29 (Task 3 Windows/Linux huggingface-hub 1.29.0). Prior: 2026-08-28 (Task 3 Windows/Linux pin trim **rejected**). Prior: 2026-08-28 (Task 3 macOS pin trim **rejected**). Prior: 2026-08-28 (Task 2 macOS Numba 0.67.0 / llvmlite 0.49.0). Prior: 2026-08-28 (Task 2 macOS torch 2.13.0 + setuptools 84.0.0). Prior: 2026-08-28 (Task 2 setuptools 84.0.0 Windows/Linux build pin). Prior: 2026-08-28 (Task 2 PyAV 18.1.0). Prior: 2026-08-28 (FileLock Linux correction + matrix). Prior: 2026-08-13. Absorbed #73 (`filelock`/`certifi`), #72 (`typing-extensions`), and #58 (`ctranslate2` 4.8.1) on `chore/deps-filelock-certifi-typing-ctranslate2`. Prior: 2026-07-14 on `chore/dependency-hygiene`.
...
| #16 | setuptools 81 → 82 (mac) | Windows/Linux/macOS build **84.0.0**; macOS `torch==2.13.0` no longer requires `setuptools<82` |
