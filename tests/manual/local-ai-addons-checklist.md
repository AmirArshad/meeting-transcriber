...
- [ ] **Normal-transcript fallback (RE-OPENED 2026-09-03 by adversarial review):** Guided failure persists an ordinary transcript plus diarization error metadata; it does not discard the meeting. **Failed 2026-09-03 (`20260903_110001`):** normal CUDA transcript persisted but `ai.diarization` was absent. **Superseded, then invalidated:** the fix that produced meetings `20260903_112216`, `20260903_112605`, and `20260903_121531` gated on `packCache.reason` / `runtimeCache.reason` without checking the owning status, which also fires when Speakrs was never installed (see the adversarial-review entry in the compatibility matrix). `Remove` sets the manifest to `notConfigured` (`diarization-setup.js:1743`) and the Remove row ran before `112216`, so those fixtures may have been recording the defect rather than the criterion. **This row must be re-run and must record `features.diarization.status` at the moment of the fallback run** — the row passes only when that field is `error` (Speakrs installed, then an artifact broken/moved), not `notConfigured`.
- [ ] **Never-installed negative case (new, required — this is the regression the review caught):** With Speakrs **never installed** (or removed, so `features.diarization.status` is `notConfigured`) and CUDA preflight otherwise ready, transcribe one meeting. Expect: ordinary CUDA transcript; **no** `ai.diarization` key in `meetings.json`; **no** "Speaker identification failed for this recording" banner in History; and **no** "Speaker identification is unavailable" line in the transcription progress log. Also confirm the same for `downloading` (transcribe while a Speakrs pack download is in flight). Cross-platform: this defect reproduced on Windows x64 and macOS arm64 too, so run the equivalent check there before release.
...
- [ ] Record hashes, device, sidecar schema, and child-process cleanup in `docs/development/V2_9_DEPENDENCY_COMPATIBILITY.md`. Stop for adversarial review before calling Task 5 formally accepted; the earlier `20260903_110001` metadata omission is retained as a superseded historical failure. **Adversarial review completed 2026-09-03 and found a shipping defect** (never-installed Speakrs reported as a per-meeting failure on Linux/Windows/macOS); the fix and the two re-opened rows above must clear before Task 5 is accepted.
