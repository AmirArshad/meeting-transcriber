...
Core Beta Omarchy remains fail-closed for add-ons. On the accepted CachyOS + RTX 4070 profile, Speakrs may un-grey only after managed CUDA 12 preflight succeeds. Linux mode is CUDA-only. Pyannote and summaries stay greyed `unsupported`. Do not treat CI CPU fixture smoke as this row.

- [ ] **Packaged setup:** Set Up Speakrs downloads the Task 4 model pack + ORT/wheel closure; Ready only after full-hash validation. `SPEAKRS_MODE=cuda`. No Hugging Face token field.
- [ ] **Validate / repair:** Validate and repair re-hash catalog pins (not `install.json` hashes) and remain Ready only when CUDA preflight still passes.
- [ ] **Guided transcription:** A new recording uses CUDA guided transcription and writes `*.speakers.json` without token values. Whisper device is recorded separately from `diarization.device`.
- [ ] **Normal-transcript fallback:** Guided failure persists an ordinary transcript plus diarization error metadata; it does not discard the meeting.
- [ ] **Cancel / quit:** Cancel and quit terminate the `speakrs-cli` process group and leave no sticky compute-queue slot.
- [ ] **Switch / Remove:** Switching engines is Speakrs-only on Linux (Pyannote setup rejected). Remove deletes only `models/diarization/speakrs` and `runtimes/speakrs-ort`; managed CUDA and Whisper caches stay.
- [ ] **Fail-closed preflight:** Missing CUDA, failed probe, or non-x64 leaves the card `unsupported` with the CUDA/x64 reason — not a supported-but-disabled control, and not CPU fallback.
- [ ] Record hashes, device, sidecar schema, and child-process cleanup in `docs/development/V2_9_DEPENDENCY_COMPATIBILITY.md`. Stop for adversarial review before calling Task 5 accepted.
