...
- [x] **Never-installed negative case (new, required — this is the regression the review caught):** With Speakrs removed, the packaged CachyOS rerun 2026-09-03 captured `features.diarization.status: "notConfigured"` for meeting `20260903_124835`, and captured `"downloading"` while a Speakrs pack setup was in flight for meeting `20260903_124956`. Both produced ordinary CUDA/`float16` transcripts; neither meeting has an `ai.diarization` key in `meetings.json`, History has no "Speaker identification failed for this recording" banner, and the transcription progress logs have no "Speaker identification is unavailable" line. **macOS arm64 rerun 2026-09-04:** fresh packaged profile meeting `20260904_093701` recorded `notConfigured` / Speakrs `cliPresent: true`, completed normal MPS/`float16` transcription, has no `ai` key, and produced no `*.speakers.json`; UI had no failure banner or speaker labels. **Windows x64 rerun 2026-09-04:** fresh packaged profile meeting `20260904_120000` recorded `notConfigured` / Speakrs `cliPresent: true`, completed normal CUDA/`float16` transcription, has no `ai` key, and produced no `*.speakers.json`; UI had no failure banner, unavailable line, or speaker labels.
...
- [x] Record hashes, device, sidecar schema, and child-process cleanup in `docs/development/V2_9_DEPENDENCY_COMPATIBILITY.md`. Evidence for the 2026-09-03 packaged Linux rerun, the 2026-09-04 macOS negative case, and the 2026-09-04 Windows x64 meeting `20260904_120000` is recorded there; the earlier `20260903_110001` metadata omission remains a superseded historical failure. Task 5 is accepted.
