...
- [ ] **Later version (v2.9, needs CachyOS + NVIDIA):** Linux CUDA Whisper; Speakrs CUDA-only; pinned Linux llama.cpp summaries. Pyannote is not a Linux option. Do not run these rows on CPU-only Omarchy.
...
- [ ] **Remove:** Linux presents only Speakrs; Pyannote has no visible selector or token field, and stale Pyannote IPC is rejected. Remove deletes only `models/diarization/speakrs` and `runtimes/speakrs-ort`; managed CUDA and Whisper caches stay.
