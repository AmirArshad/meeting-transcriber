...
- [ ] **Still open (pre-merge review 2026-08-28): tray icon is visibly rendered.** The Phase 4 row above proved the SNI item *registered*, not that an icon was *drawn* — and it was not: Linux resolved the idle image to Windows `build/icon.ico`, which `nativeImage.createFromPath` decodes to an **empty** image outside Windows, while `new Tray(emptyImage)` still succeeds. Fixed by dedicated PNGs (`build/iconTrayLinux.png`, `build/iconTrayLinuxRecording.png`). Confirm on an Omarchy panel: idle shows the app mark; during recording it shows the app mark with the red REC badge; the context menu still opens.
- [ ] **Still open (pre-merge review 2026-08-28): unplugged-endpoint filtering.** `is_pulse_port_unavailable` never matched a real `pulsectl.EnumValue` until this review, so the "omit `available=no` sinks/monitors" rule was dead code. Re-run the Phase 1 HDMI case (force `output:hdmi-stereo` with no cable) and confirm the sink and its `.monitor` are absent from both dropdowns, then restore the profile to `off`.
