...
**Plan (binding — read first):** [docs/superpowers/plans/2026-07-16-speakrs-diarization-migration.md](docs/superpowers/plans/2026-07-16-speakrs-diarization-migration.md) — **v5**. Exclusive user selector (both engines stay). Frozen CLI contract, delete lists, copy, defaults. speakrs **0.5.0**. Mac packaged soak 2026-08-14: setup/CoreML path works; selector UX + 2-talker over-cluster are Task 7a.
...
- [ ] [Risk: High] **Task 7 — soak on merged builds:** no default flip; README speaker-ID **8 GB min / 16 GB recommended** + Windows ~823 MB disk note; benchmarks → `docs/development/SPEAKRS_BENCHMARKS.md`; bar = ≥25 meetings (≥10/OS), Mac 2×50-turn A/B ≤ +2, selector/switch checklist green. **Mac 2026-08-14:** packaged dir build Ready + CoreML guided (no crash). Selector checklist **not** green. Speakrs 35 s in-room+YouTube clip reported **3** speakers vs pyannote **2** on a same-morning 26 s clip of the same kind. Windows CUDA soak still outstanding.
- [ ] [Risk: Medium] **Task 7a — selector UX + Mac quality follow-up (blocking ship bar):** Settings/Home button **Switch model** when the other engine is installed (not Set Up). Reproduce and fix Speakrs→Pyannote after Speakrs is Ready. `.ai-addon-field[hidden] { display: none }` so token + Speakers dropdown hide on Speakrs (helpers already return false; CSS `display: flex` wins today). Speakrs stays auto-only (0.5.0 has no speaker-count API). Same-audio pyannote vs Speakrs A/B on the 10:22 clip → `SPEAKRS_BENCHMARKS.md`. Do not invent `--speaker-count`. Validation: `npm test` + packaged Mac checklist rows.
...
1. **Speakrs diarization migration** (above) — Task 7a selector UX next, then remaining Mac/Windows soak. No silent cutover.
