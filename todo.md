...
- [ ] [Risk: High] On this Mac, bump stale `requirements-macos-build.txt` pins to current resolve (keep the lock; do not float). Clusters: `huggingface-hub` 1.29.0 + pulled transitives; `typer` 0.27.2 (drop macOS `click` if unused; `colorama` Windows-only); `mlx` 0.32.2 after packaged MLX smoke. Fix breaking changes in code. Do not bump PyObjC or `sounddevice` unless capture gates pass.
