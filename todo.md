...
- [ ] [Risk: High] On this Mac, bump stale `requirements-macos-build.txt` pins to current resolve (keep the lock; do not float). **Clusters 1–3 accepted** (huggingface-hub 1.29.0; typer 0.27.2 / drop unused macOS colorama / keep click; mlx 0.32.2). Remaining: macOS-only floats (`cffi`, `regex`, `Pygments`, `annotated-doc`) if not already pulled. Do not bump PyObjC or `sounddevice` unless capture gates pass.
