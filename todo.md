...
- [ ] [Risk: Medium] **Phase 2 — runtime + device plumbing:** `requirements-linux.txt` (`pulsectl==24.12.0`, `SoundCard==0.4.6`); verified Linux Python/ffmpeg pins into `build/download-manifest.js`; opaque `pulse-source:`/`pulse-monitor:`/`none` device IDs; `parseInt` coercions removed. **In progress on `release/linux`.** Packaged-runtime “enumerate the same devices” still needs a prepared Python on a Pulse host.
