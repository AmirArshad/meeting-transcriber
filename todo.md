...
- [x] [Risk: High] Task 2: add the hardened, catalog-pinned Linux-x64 CUDA 12 runtime (disabled by default). Offline hash-verified install, contained `LD_LIBRARY_PATH`, structured NVIDIA probe, CTranslate2 CUDA init, `LINUX_CUDA_DEVICE_MISMATCH`, x64 enforcement, and tombstone uninstall. Do not start Speakrs, Pyannote, summaries, or Task 3 packaged acceptance. Linux AI stays disabled until Task 3 packaged RTX 4070 evidence.
