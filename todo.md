...
Branch: `refactor/codebase-phase-3c` (Phase 3c: transcription + summary + recorder lifecycle; depends on merged 3b #36).
...
- [x] [Risk: High] Phase 3c: recorder/transcription/summary lifecycle last; gated on Phase 0.2 and 0.4. Extracted `transcription-service.js`, `summary-service.js`, `recorder-service.js` via Pattern C. `download-model` remains off the compute queue. Preload/renderer untouched. Manual recording/transcription smoke still recommended before merge; batch macOS with Phase 7 when needed.
