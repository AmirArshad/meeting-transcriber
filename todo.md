...
- [x] [Risk: Low] **Phase 0 — baseline + characterization:** diagnosable-assertion fix in `tests/js/speakrs-task2-hardening.test.js` (snippet in plan Gate A); Linux cases in platform-selection tests; `npm test` green on the Omarchy host; Linux rows in both manual checklists; no Linux feature advertised as ready. **Landed on `release/linux`.**
