...
- 2026-05-17: Fixed packaged macOS summary validation after Qwen3.5 setup completed but `llama-cli` smoke validation timed out and left a truncated command-path error. The smoke test now uses a tiny non-interactive `llama.cpp` invocation (`--ctx-size 512`, `--predict 1`, `--no-warmup`, `--single-turn`, `--simple-io`) and reports sanitized timeout errors. Local installed model checksum matched the catalog, runtime reinstall reused the existing model, exact backend validation passed, and `npm run test:python` passed.
- 2026-05-17: Reviewed summary chunking/performance strategy. Summary generation now runs `llama-cli` in non-interactive single-turn/simple-IO mode with reasoning disabled, strips echoed prompts before JSON parsing, uses more of the 32k context window for chunk budgets, and single-chunk transcripts skip the redundant final merge prompt while multi-chunk long meetings keep the existing map-reduce flow. `npm test` and `npm run test:python` passed.
- 2026-05-17: Reviewed `pyannote/speaker-diarization-community-1` examples against our diarization path. Existing implementation already follows local Community-1 guidance for GPU/MPS `.to(...)`, user token setup, `num_speakers`, and `exclusive_speaker_diarization`. Added in-memory prepared-audio execution via `torchaudio.load`, local-cache-only model loading during actual diarization, and managed PyTorch CUDA DLL bin discovery for Windows diarization subprocesses. Targeted diarization and main-process helper tests passed.
- 2026-05-17: Applied Claude review hardening for the summary/diarization optimization pass. Prompt-echo stripping now only removes early `llama-cli` echoes after the input marker, summary chunking reserves more context for prompt/schema overhead, missing offline pyannote cache errors now tell users to re-run setup, in-memory diarization input is duration-gated with a long-recording file-path fallback, and managed diarization CUDA DLL PATH expansion is scoped to diarization subprocesses.
- 2026-05-17: Followed up on Claude review nits by memoizing the managed diarization site-packages lookup with invalidation before validation/removal, documenting PCM-only duration probing and the prompt-echo start-of-output cap, making the chunk/merge summary test force chunking through the runtime context instead of incidental text length, and adding coverage for non-`OSError` offline pyannote cache load failures. Deferred catalog-level `--reasoning off` parameterization intentionally until a non-Qwen3 summary model is added.
- 2026-05-17: Addressed Claude review on summary cancellation and text decoding. Summary generation cancellation is now registered in the main process before preflight work, metadata writes are no longer killed after summary sidecars exist, renderer cancel state resets when IPC reports no cancellable job, meeting history uses replacement UTF-8 decoding to match summary transcript hashing, legacy pyannote `use_auth_token` loaders are rejected for offline execution, CSS loading/cancel behavior is documented, and the manual smoke checklist includes immediate summary-cancel validation.
- 2026-05-18: Investigated Windows summary runtime setup slowness/cancel failures. Windows runtime setup legitimately downloads about 610 MB of pinned CUDA runtime archives versus the 8 MB macOS Metal archive, but the shared HTTPS downloader was also emitting progress on every chunk and did not destroy the active response stream on cancellation. Runtime/model artifact downloads now throttle progress callbacks, destroy the response stream plus request, and keep partial-file cleanup best-effort for Windows file locks. Focused `node --test tests/js/ai-addon-setup.test.js` passed.
- 2026-05-18: Fixed the Windows speaker setup `torchvision` runtime failure by adding the matching `torchvision==0.23.0+cu126` pin to the managed CUDA dependency set, invalidating already-installed dependency markers whose pinned requirements no longer match, and changing setup error recovery copy so local runtime import failures point to reinstall/validate instead of misleading token/model-terms guidance. Existing Windows diarization users should be told in release notes that this update asks them to remove and reinstall speaker setup once. Focused JS tests, syntax checks, `git diff --check`, and `npm test` passed.
- 2026-05-18: Windows smoke passed for summary setup and diarization. Follow-up cleanup prevents installed-model runtime paths from probing Hugging Face unnecessarily: faster-whisper now uses `local_files_only`/HF offline mode when its cache is present, guided transcription applies the same cache-present Whisper policy, MLX avoids importing/downloading through `huggingface_hub` when required files are cached, diarization already loads pyannote with `local_files_only=True`, and summary generation launches with offline HF env. Explicit setup/download paths can still fetch missing models.
- 2026-05-18: Hardened archive extraction and transcription cache docs: macOS `tar.gz` runtime setup now extracts in a worker thread, zip/tar traversal checks are shared via `src/ai-addon-archive-helpers.js`, worker exit without a result fails closed, model download/preload pass the Whisper cache dir without forcing offline mode, and AGENTS.md documents the transcription cache contract across JS/Python.
...
- [x] Archive root `CLAUDE.md` under `docs/internal/archive/` so Cursor does not double-load agent context; use root `AGENTS.md` for OpenCode and Cursor (see `.cursor/rules/`).
