...
- [ ] [Risk: High] On this Mac, bump stale `requirements-macos-build.txt` pins to current resolve (keep the lock; do not float). **Cluster 1 accepted** (2026-08-28): macOS `huggingface-hub==1.29.0` + pulled transitives (`hf-xet` 1.6.0, `click` 8.5.0, `fsspec`, `anyio`, `charset-normalizer`, `idna`, `packaging`, `tqdm`); `filelock` stays 3.32.3. Remaining: `typer` 0.27.2 (`click` stays — hub 1.29 requires it; drop macOS `colorama` only if unused); `mlx` 0.32.2 after packaged MLX smoke; remaining macOS-only floats. Do not bump PyObjC or `sounddevice` unless capture gates pass.
