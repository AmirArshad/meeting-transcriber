...
**Plan (binding — read first):** [docs/superpowers/plans/2026-07-16-speakrs-diarization-migration.md](docs/superpowers/plans/2026-07-16-speakrs-diarization-migration.md) — **v3**. Frozen CLI contract, execution guardrails, per-task file lists and validation commands. speakrs **0.5.0** is still the latest crate.
**Rules that get the best result:** never start task N+1 with task N's validation red; `npm test && npm run test:python` green at every task boundary; everything stays additive until Task 8; if a pinned snapshot test fails before Task 8, the change is wrong — not the snapshot. **Do not flip `DIARIZATION_ENGINE` to speakrs until Task 7.**

- [ ] [Risk: Low] **Task 0a — macOS spike (tables only, no GO):** rustc 1.88+; speakrs 0.5.0 with `default-features=false`, `default-linalg`+`coreml`, `online` off; `ExecutionMode::CoreMl` (not the CPU example); record `required_files(CoreMl)` from source + sizes; 3 internal meetings vs pyannote MPS; CLI + combined RSS; notes in `docs/development/SPEAKRS_SPIKE_NOTES.md`. No app code touched.
- [ ] [Risk: Medium] **Task 0b — Windows spike (gate):** `default-linalg`+`cuda`+`load-dynamic`; prove ORT DLL closure against existing cuda12 pip DLLs on a box that already runs pyannote CUDA; finish tables. **Explicit GO/NO-GO only after this task.**
- [ ] [Risk: Low] **Task 1 pre-step — characterization golden tests** (may land in parallel with 0a): pin `*.speakers.json` field sets + today's `annotationSource` values, `emit_progress` **per entry point**, engine-agnostic payload **shape** (not full argv). Green against unmodified code; stay green through Task 7.
- [ ] [Risk: Low] **Task 1 — `speakrs-cli` crate** to the frozen JSON contract (`annotationSource` = `exclusive_speaker_diarization`) + fixture WAV + contract tests. Features: always keep `default-linalg`. Validation: `cargo test`, `cargo clippy -- -D warnings`, `npm run test:python`.
- [ ] [Risk: Medium] **Task 2 — model pack + catalog:** repack script, `DIARIZATION_ENGINE` default **`pyannote`**, speakrs catalog entry, engine-aware `setupComplete` / status / storage (no new facade exports); token-free setup reaches `ready` **and** `setupComplete`. Validation: `npm test`.
...
- [ ] [Risk: High] **Task 3 — Python engine dispatch:** `--engine` flag, `speakrs_runner.py` with CLI in the **same** POSIX process group (no `start_new_session`), inner progress phases only, no torch assert on speakrs path, redaction fix at `guided_transcription.py:492`. Validation: `npm run test:python` incl. kill-propagation test.
- [ ] [Risk: High] **Task 4 — main-process plumbing:** spawn env/args, token-free validation path (branch — keep pyannote `--token-stdin`), packaged CLI resolution. Do not require `runtime.modelRef`. Smallest possible diff in `transcription-service.js`.
- [ ] [Risk: Medium] **Task 5 — renderer UX + migration:** hide token UI for speakrs, legacy-pyannote prompt only after Task 7 flip, explicit legacy cleanup; hide speaker-count if auto-only; update `tests/manual/local-ai-addons-checklist.md`.
- [ ] [Risk: Medium] **Task 6 — build/CI/release packaging:** `buildSpeakrsCli`, extraResources/codesign, **stage validation fixture WAV**, CI Rust + CPU smoke (Windows `cuda,load-dynamic` compile-only), release workflow. Validation: `npm run prepare-build`, packaged smokes, `npm run test:all`. **Merge the branch after this task; default still pyannote.**
- [ ] [Risk: High] **Task 7 — soak on merged builds + cutover:** flip `DIARIZATION_ENGINE` to `'speakrs'` + update `AGENTS.md`; benchmark matrix → `docs/development/SPEAKRS_BENCHMARKS.md`; bar = ≥25 meetings (≥10/OS), 2×50-turn A/B ≤ +2 vs pyannote, checklist green on packaged Windows CUDA + macOS AS.
