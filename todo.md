...
- [ ] [Risk: High] On this Mac, bump stale `requirements-macos-build.txt` pins to current resolve (keep the lock; do not float). **Cluster 1 accepted** (huggingface-hub 1.29.0 + transitives). **Cluster 2 accepted** (typer 0.27.2; macOS colorama pin removed as unused; click kept for hub 1.29). Remaining: `mlx` 0.32.2 after packaged MLX smoke; remaining macOS-only floats. Do not bump PyObjC or `sounddevice` unless capture gates pass.
