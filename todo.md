...
- [ ] [Risk: Medium] **Task 2 — model pack + catalog:** repack (CUDA list + tail extras; ORT **1.27.1** cuda12 archive); both catalog entries; manifest `engine` (new → **speakrs**, legacy pyannote kept); engine-aware `setupComplete`; exclusive delete helpers. Token-free speakrs reaches `ready` **and** `setupComplete`. Validation: `npm test`.
...
- [ ] [Risk: High] **Task 4 — main-process plumbing:** `--engine` from manifest; speakrs env + Windows ORT PATH; token-free validation branch (keep pyannote `--token-stdin`). Do not require `runtime.modelRef`.
- [ ] [Risk: Medium] **Task 5 — selector UI + exclusive switch:** two cards (Mac Speakrs **Recommended**); confirm + delete the other engine; hide token UI for speakrs; hide speaker-count; checklist rows for switch both ways. No new IPC channel (`setup-diarization({ engine })`).
- [ ] [Risk: Medium] **Task 6 — build/CI/release packaging:** `buildSpeakrsCli`, extraResources/codesign, fixture WAV, CI Rust + CPU smoke (Windows `cuda,load-dynamic` compile-only). Validation: `npm run prepare-build`, `npm run test:all`. **Merge after this task; both engines selectable.**
