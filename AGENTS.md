...
- Linux is in active development (`docs/initiatives/LINUX_SUPPORT.md`, branch `release/linux`). Phase 3 wires `audio.linux_recorder` with opaque Pulse device IDs (`pulse-source:<name>`, `pulse-monitor:<name>`, `pulse-sink:<name>`, `none`); renderer and IPC must not `parseInt` them. Phase 3 Omarchy hardware smoke closed 2026-08-27 (60-minute soak cancelled, not run). Phase 4 product code closed 2026-08-27: Linux `password-store` + `--ozone-platform-hint=auto` (native Wayland on Hyprland; secret backend `gnome_libsecret`), SNI tray with `setContextMenu` only, CPU faster-whisper, add-ons greyed `unsupported`. **NEXT is the Phase 4 adversarial review** (parked prompt), then Phase 5 packaging. UI copy must not advertise Linux CUDA or add-ons as ready.
