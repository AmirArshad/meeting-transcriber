...
- Linux is in active development (`docs/initiatives/LINUX_SUPPORT.md`, branch `release/linux`). Phase 3 wires `audio.linux_recorder` with opaque Pulse device IDs (`pulse-source:<name>`, `pulse-monitor:<name>`, `pulse-sink:<name>`, `none`); renderer and IPC must not `parseInt` them. Phase 3 Omarchy hardware smoke closed 2026-08-27 (60-minute soak cancelled, not run). Phase 4 product code and adversarial review closed 2026-08-27: Linux `password-store` + `--ozone-platform-hint=auto` (native Wayland on Hyprland; secret backend `gnome_libsecret`), SNI tray with `setContextMenu` only and taskbar-visible fallback when tray creation fails, CPU faster-whisper, add-ons greyed and fail-closed `unsupported`. **Phase 5 packaging landed 2026-08-28** (electron-builder 26.x `toolsets.appimage` `1.0.2`, x86_64 AppImage + pacman `AvaNevis-Setup-*`, Ubuntu CI, updater matching). Gate B still blocks `.github/workflows/build-release.yml`. Core Beta is **not** marked complete: packaged Settings / legal-notices UI clicks and a packaged recording session were not run. UI copy must not advertise Linux CUDA or add-ons as ready. Do not start Phases 6–9.
