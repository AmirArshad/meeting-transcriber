...
- `src/main/`: extracted main-process services wired from `src/main.js` (Phase 3a–3c). The Phase 0 source-scan tests treat `src/main.js` + `src/main/**/*.js` as one combined main-process surface, so IPC channel names/payloads stay pinned across the split:
  ...
  - `src/main/recorder-service.js`: `registerRecorderService(ipcMain, deps)` / `createRecorderService(deps)`; owns recording lifecycle state (`pythonProcess`, stop/heartbeat/power-save) and registers `run-recording-preflight`, `start-recording`, `stop-recording`, `cancel-recording`, `get-recording-state`; publishes authoritative `starting`/`recording`/`stopping`/`cancelling`/`idle` via optional `onCaptureStateChanged`; exports quit-during-recording helpers for `before-quit`. Quit cancel after `stop` was sent must await/persist the stop result (never claim "recording continues"). `quitCommitted` rejects new `start-recording` / `generate-summary` once a quit drain has begun. Discard (`cancel-recording`) never calls `addMeetingToHistory` / transcription enqueue; stop and cancel are mutually exclusive (first command wins).
...
- Stdin control is exact-token only (`line.strip().lower()` is exactly `stop` or `cancel` — not substring match). `cancel` skips Stage A / `finalize_capture`, tombstones the capture manifest as `discarded` before best-effort spool cleanup, and emits structured `{ success: true, cancelled: true }` (never stderr-only exit). Crash before the discarded marker remains recoverable as a normal interrupted capture.
...
- Discarded capture sessions (`state: discarded`) are cleanup-only for discovery/scan-import — never resurrected as meetings.
- Renderer cancel calls include the known recording `sessionId`; main rejects a mismatched ID with `RECORDING_SESSION_MISMATCH` so a stale Start continuation cannot cancel a newer session. Cancel during pre-spawn startup may omit the ID and remains generation-guarded.
- Recorder startup retains its 15s/10s wall clock even while the startup-cancel callback is installed. Cancel uses the stop wall-clock budget plus bounded post-kill settlement; a child that does not close must not leave the cancel promise permanently latched, while capture presence remains `cancelling` until process truth changes.
- Manifest-less `*.capture` cleanup probes `session.lock` with `timeout=0`, re-checks `manifest.json`, releases the lock, then deletes. Releasing before `rmtree` is required on Windows because an open lock file cannot be removed.
...
- Whisper model download / preload (`download-model`) — stays **off** the compute queue (must not enqueue). **Phase 2 between-job admission:** enqueue on `gpuResourceActionQueue` so preload runs in the gap after the active transcription releases the resource queue and before the next job enters it (no `MODEL_DOWNLOAD_COMPUTE_BUSY` fail-fast; do not use a 15-minute idle wait). `cancel-download-model` aborts an in-flight preload. Non-zero preload exits must re-check cache completeness before reporting success.
- AI add-on setup downloads (`aiAddonActionQueue`) — separate serialization from compute.
- GPU runtime install/repair/uninstall (`gpuRuntimeActionPromise`) — separate lock from the compute queue, but **mutually exclusive with active compute and Whisper preload** through the composition-root `gpuResourceActionQueue`: `runGpuRuntimeAction` admits between jobs on that resource queue (Phase 2; no `GPU_RUNTIME_COMPUTE_BUSY` fail-fast / no 15-minute idle wait) so pip cannot race loaded CUDA DLLs. Compute actions must rely on this FIFO rather than awaiting a later `gpuRuntimeActionPromise` while holding the resource slot. Parked preload/runtime actions re-check `quitCommitted` at execution time. Destructive AI add-on removal rejects while compute/preload/runtime work is pending, then synchronously reserves the same resource queue before deleting files; it must not wait unbounded behind active work or begin later during quit teardown.
...
- Whisper `download-model` idle-wait constant (`AI_COMPUTE_TIMEOUT_MS.modelDownloadIdleWait`, 15 minutes): retained for documentation; Phase 2 uses between-job `gpuResourceActionQueue` admission instead of idle-wait or fail-fast
...
- GPU runtime vs compute idle-wait constant (`AI_COMPUTE_TIMEOUT_MS.gpuRuntimeComputeIdleWait`, 15 minutes): retained for documentation; Phase 2 between-job admit uses `gpuResourceActionQueue` serialization instead
...
`runWallClockComputeAction` must terminate processes registered after timeout/quit/settlement, including a transcription child spawned after an awaited CUDA probe during timeout settle grace. Queue-state payloads carry a monotonically increasing `seq`; renderer init snapshots and pushes with `seq <= lastAppliedTranscriptionQueueSeq` are ignored completely, including terminal-transition side effects.
